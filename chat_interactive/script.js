...
let current_chat = 0; //Initialize variable that matches the number of the current chat loaded
...
function addClass() { //Adds a new class to be selected from the list of classes. Will later also add class to profile.
  ...
  class_count++;
  let child = document.createElement('div'); //Prepare value to attach to HTML
  child.innerHTML = `<input type="button" name="class_${class_count-1}_button" value="Class ${class_count-1}" id="class_button" onclick="loadMessages(${class_count-1})">`; //Write button to child, subtracts one from class_count to start counting at 0
  class_list_obj.appendChild(child); //Append HTML for class
}

function loadMessages(class_number) { //Load the messages from the current chat into the message window.
  message_obj.innerHTML = ""; //Clear out contents of messages
  current_chat = class_number; //Set current chat to correspond to the class number needed
  if (message_list[current_chat] == undefined) { //If no messages are saved for current chat...
    let child = document.createElement('div'); //Prepare value to attach to HTML
